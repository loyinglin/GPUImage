...
- (void)viewDidLoad {
    ...
    /*
     大神可以打开 PLCameraStreamingKit 那个目录  里面有.m  大神顺便看一个我mark 的地方 在下面。
...
#pragma mark   ------ 注意一下 这个地方
            /*
             addGPUImageFilter  这是个接口 不过好像只能传 GPUImageFilter类型的
             
             如果是GPUImageBeautifyFilter的话 美颜效果没有用
             就像这样会报错的。 */
             GPUImageBeautifyFilter *filter = [[GPUImageBeautifyFilter alloc] init];
             [self.session addGPUImageFilter:(GPUImageFilter *)filter];
            
            
//            
//            GPUImageToneCurveFilter *filter = [[GPUImageToneCurveFilter alloc] initWithACV:@"cross_1"];
//            [self.session addGPUImageFilter:filter];
